...
dist: dist-deps $(TARGET) bower.json ## Prepare JS files for distribution
	...
	cp LICENSE dist/
	uglifyjs shim.js $(UGLIFYOPTS) -o dist/shim.min.js # -b beautify=false,preamble "$$(head -n 1 bits/00_header.js)"
	uglifyjs $(DISTHDR) dist/$(TARGET) $(UGLIFYOPTS) -o dist/$(LIB).min.js # --source-map dist/$(LIB).min.map # -B --preamble "$$(head -n 1 bits/00_header.js)"
	misc/strip_sourcemap.sh dist/$(LIB).min.js
	uglifyjs $(DISTHDR) $(REQS) dist/$(TARGET) $(UGLIFYOPTS) -o dist/$(LIB).core.min.js #--source-map dist/$(LIB).core.min.map # -B --preamble "$$(head -n 1 bits/00_header.js)"
	misc/strip_sourcemap.sh dist/$(LIB).core.min.js
	uglifyjs $(DISTHDR) $(REQS) $(ADDONS) dist/$(TARGET) $(AUXTARGETS) $(UGLIFYOPTS) -o dist/$(LIB).full.min.js #--source-map dist/$(LIB).full.min.map # -b beautify=false,preamble "$$(head -n 1 bits/00_header.js)"
	uglifyjs $(DISTHDR) $(MINITGT) $(UGLIFYOPTS) -o dist/$(LIB).mini.min.js # --source-map dist/$(LIB).mini.min.map -B -preamble "$$(head -n 1 bits/00_header.js)"
